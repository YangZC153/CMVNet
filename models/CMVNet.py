...
class FuseImages3D(nn.Module):
    ...
    def forward(self, SAFE_Feature_Map_X, SAFE_Feature_Map_Y, SAFE_Feature_Map_Z):
        weighted_SAFE_Feature_Map_X = self.attention_module(SAFE_Feature_Map_X)
        weighted_SAFE_Feature_Map_Y = self.attention_module(SAFE_Feature_Map_Y)
        weighted_SAFE_Feature_Map_Z = self.attention_module(SAFE_Feature_Map_Z)
        HCSA_output = weighted_SAFE_Feature_Map_X + weighted_SAFE_Feature_Map_Y + weighted_SAFE_Feature_Map_Z
        HCSA_output = self.dropout(HCSA_output)
        return HCSA_output
...
class CMVNet(nn.Module):
    ...
    def forward(self, x, emb_codes):
        ...
        SAFE_Feature_Map_Z = self.enc0_x(x)
        SAFE_Feature_Map_Y = self.enc0_y(x)
        SAFE_Feature_Map_X = self.enc0_z(x)
        HCSA_output = self.fuse_images_layer(SAFE_Feature_Map_X, SAFE_Feature_Map_Y, SAFE_Feature_Map_Z)
        SAFE_Feature_Map_1 = self.ec0(x)
        h = self.enc0_(cat([HCSA_output, SAFE_Feature_Map_1], dim=1))

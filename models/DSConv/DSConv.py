...
class DCN_Conv(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size, extend_scope, morph, if_offset, device):
        ...
        self.dcn_conv_x = nn.Conv3d(in_ch, out_ch, kernel_size=(1, 1, kernel_size), stride=(1, 1, kernel_size), padding=0)  # [w_z, w_y, w_x * kernel_size]  ==》 [w_z, w_y, w_x]
        self.dcn_conv_y = nn.Conv3d(in_ch, out_ch, kernel_size=(1, kernel_size, 1), stride=(1, kernel_size, 1), padding=0)  # [w_z, w_y * kernel_size, w_x]  ==》 [w_z, w_y, w_x]
        self.dcn_conv_z = nn.Conv3d(in_ch, out_ch, kernel_size=(kernel_size, 1, 1), stride=(kernel_size, 1, 1), padding=0)  # [w_z * kernel_size, w_y, w_x]  ==》 [w_z, w_y, w_x]

        self.dcn_conv = nn.Conv3d(in_ch, out_ch, kernel_size=kernel_size, stride=kernel_size, padding=0)  # [w_x, w_y, w_z]  ==》 [w_x/kernel_size, w_y/kernel_size, w_z/kernel_size]
        self.gn = nn.GroupNorm(out_ch // 4, out_ch)
        self.relu = nn.ReLU(inplace=True)
...
class DCN(object):
    ...
    def _coordinate_map_3D(self, offset, if_offset):
        ...
        if self.morph == 0:
            ...
            if if_offset:
                ...
                z_new = z_new.reshape([self.num_batch, 1, 1, self.num_points, self.depth, self.width, self.height])  # [N, 1, 1, num_points, d, w, h]
                z_new = z_new.permute(0, 4, 1, 5, 2, 6, 3)  # [N, d, 1, w, 1, h, num_points]
                z_new = z_new.reshape([self.num_batch, self.depth, self.width, self.num_points*self.height])  # [N, d, w, h * num_points]
                ...
                y_new = y_new.reshape([self.num_batch, self.depth, self.width, self.num_points*self.height])  # [N, d, w, h * num_points]
                ...
                x_new = x_new.reshape([self.num_batch, self.depth, self.width, self.num_points*self.height])  # [N, d, w, h * num_points]
            ...
            z_new = z_new.reshape([self.num_batch, self.depth, self.num_points * self.width, self.height])  # [N, d, w * num_points, h]
            y_new = y_new.reshape([self.num_batch, 1, self.num_points, 1, self.depth, self.width, self.height])
            y_new = y_new.permute(0, 4, 1, 5, 2, 6, 3)
            y_new = y_new.reshape([self.num_batch, self.depth, self.num_points * self.width, self.height])  # [N, d, w * num_points, h]
            x_new = x_new.reshape([self.num_batch, 1, self.num_points, 1, self.depth, self.width, self.height])
            x_new = x_new.permute(0, 4, 1, 5, 2, 6, 3)
            x_new = x_new.reshape([self.num_batch, self.depth, self.num_points * self.width, self.height])  # [N, d, w * num_points, h]
            return z_new, y_new, x_new
            ...
            z_new = z_new.permute(0, 4, 1, 5, 2, 6, 3)
            z_new = z_new.reshape([self.num_batch, self.num_points*self.depth, self.width, self.height])  # [N, d * num_points, w, h]

            y_new = y_new.reshape([self.num_batch, self.num_points, 1, 1, self.depth, self.width, self.height])
            y_new = y_new.permute(0, 4, 1, 5, 2, 6, 3)
            y_new = y_new.reshape([self.num_batch, self.num_points*self.depth, self.width, self.height])  # [N, d * num_points, w, h]

            x_new = x_new.reshape([self.num_batch, self.num_points, 1, 1, self.depth, self.width, self.height])
            x_new = x_new.permute(0, 4, 1, 5, 2, 6, 3)
            x_new = x_new.reshape([self.num_batch, self.num_points*self.depth, self.width, self.height])  # [N, d * num_points, w, h]
            return z_new, y_new, x_new
    ...
    def _bilinear_interpolate_3D(self, input_feature, z, y, x):
        ...
        index_d0 = base_y0 + base_z1 - base + x1

        index_a1 = base_y1 + base_z0 - base + x0
        index_b1 = base_y1 + base_z1 - base + x0
        index_c1 = base_y1 + base_z0 - base + x1
        index_d1 = base_y1 + base_z1 - base + x1

        value_a0 = input_feature_flat[index_a0.type(torch.int64)].cuda()
        value_b0 = input_feature_flat[index_b0.type(torch.int64)].cuda()
        value_c0 = input_feature_flat[index_c0.type(torch.int64)].cuda()
        value_d0 = input_feature_flat[index_d0.type(torch.int64)].cuda()
        value_a1 = input_feature_flat[index_a1.type(torch.int64)].cuda()
        value_b1 = input_feature_flat[index_b1.type(torch.int64)].cuda()
        value_c1 = input_feature_flat[index_c1.type(torch.int64)].cuda()
        value_d1 = input_feature_flat[index_d1.type(torch.int64)].cuda()  # [N*KD*KW*KH, C]
        ...
        if self.morph == 0:
            outputs = outputs.reshape([self.num_batch, self.depth, self.width, self.num_points*self.height, self.num_channels])  # [B, d, w, num_points * h, c]
            outputs = outputs.permute(0, 4, 1, 2, 3)  # [B, c, d, w, num_points * h]
        elif self.morph == 1:
            outputs = outputs.reshape([self.num_batch, self.depth, self.num_points*self.width, self.height, self.num_channels])  # [B, d, num_points * w, h, c]
            outputs = outputs.permute(0, 4, 1, 2, 3)  # [B, c, d, num_points * w, h]
        else:
            outputs = outputs.reshape([self.num_batch, self.num_points*self.depth, self.width, self.height, self.num_channels])  # [B, num_points * d, w, h, c]
            outputs = outputs.permute(0, 4, 1, 2, 3)  # [B, c, num_points * d, w, h]

...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("-c", "--config", default="/home/yangzhencheng/CMVNet/configs/CMVNet/seg-CMVNet_test.yaml", required=False)  
    ...
    if config.trainer.do_test:
        ...
        results = experiment.test(phase="Test", alpha=2, save_individual_results=True)

        (test_iou, test_dice, test_hausdorff_distance, test_hausdorff_distance_95, test_smoothness, test_small_volume_detection,
        test_surface_dice, test_ncc,
        right_iou, right_dice, right_hausdorff_distance, right_hausdorff_distance_95, right_smoothness, right_small_volume_detection,
        right_surface_dice, right_ncc,
        left_iou, left_dice, left_hausdorff_distance, left_hausdorff_distance_95, left_smoothness, left_small_volume_detection,
        left_surface_dice, left_ncc,
        ...
        logging.info(f'\nTest results (Overall) - IoU: {test_iou}, Dice: {test_dice}, Hausdorff Distance: {test_hausdorff_distance}, '
                    ...
                    f'Surface Dice: {test_surface_dice}, NCC: {test_ncc}')
        ...
        logging.info(f'\nTest results (Right) - IoU: {right_iou}, Dice: {right_dice}, Hausdorff Distance: {right_hausdorff_distance}, '
                    ...
                    f'Surface Dice: {right_surface_dice}, NCC: {right_ncc}')
        ...
        logging.info(f'\nTest results (Left) - IoU: {left_iou}, Dice: {left_dice}, Hausdorff Distance: {left_hausdorff_distance}, '
                    ...
                    f'Surface Dice: {left_surface_dice}, NCC: {left_ncc}')

        logging.info(f'\n{"="*80}')
        logging.info(f'Individual Sample Results (Total: {len(individual_results)} samples)')
        logging.info(f'{"="*80}')

        for idx, sample in enumerate(individual_results):
            logging.info(f'\n【Sample {idx+1}】: {sample["subject_name"]}')
            logging.info(f'┌─ Overall Metrics:')
            logging.info(f'│  IoU: {sample["iou"]:.4f}, Dice: {sample["dice"]:.4f}')
            
            if 'hausdorff_distance' in sample:
                logging.info(f'│  HD: {sample["hausdorff_distance"]:.4f}, HD95: {sample["hausdorff_distance_95"]:.4f}')
                logging.info(f'│  Smoothness: {sample["smoothness"]:.4f}, Small Vol: {sample["small_volume_detection"]}')
                logging.info(f'│  Surface Dice: {sample["surface_dice"]:.4f}, NCC: {sample["ncc"]:.2f}')
            
            if 'right_iou' in sample:
                logging.info(f'├─ Right IAN Metrics:')
                logging.info(f'│  IoU: {sample["right_iou"]:.4f}, Dice: {sample["right_dice"]:.4f}')
                logging.info(f'│  HD: {sample["right_hausdorff"]:.4f}, HD95: {sample["right_hausdorff_95"]:.4f}')
                logging.info(f'│  Smoothness: {sample["right_smoothness"]:.4f}, Small Vol: {sample["right_small_vol"]}')
                logging.info(f'│  Surface Dice: {sample["right_surface_dice"]:.4f}, NCC: {sample["right_ncc"]:.2f}')
                
                logging.info(f'└─ Left IAN Metrics:')
                logging.info(f'   IoU: {sample["left_iou"]:.4f}, Dice: {sample["left_dice"]:.4f}')
                logging.info(f'   HD: {sample["left_hausdorff"]:.4f}, HD95: {sample["left_hausdorff_95"]:.4f}')
                logging.info(f'   Smoothness: {sample["left_smoothness"]:.4f}, Small Vol: {sample["left_small_vol"]}')
                logging.info(f'   Surface Dice: {sample["left_surface_dice"]:.4f}, NCC: {sample["left_ncc"]:.2f}')

        logging.info(f'\n{"="*80}')
        logging.info(f'Results saved to CSV and JSON files in {os.path.join(experiment.config.project_dir, experiment.config.title)}')
        logging.info(f'{"="*80}\n')
    ...
    #     experiment.load()
    #     experiment.inference(os.path.join(config.data_loader.dataset,'Inference'))